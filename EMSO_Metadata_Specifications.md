...
The DIGI4ECO Metadata Specification for ERDDAP defines the essential metadata elements required for datasets to achieve 
compliance within the DIGI4ECO research infrastructure. These specifications ensure consistency, interoperability, and 
discoverability of DIGI4ECO data, supporting long-term ocean observation and multidisciplinary research. Rooted in widely
...
The structure of DIGI4ECO-compliant datasets is depicted in the following picture:

<p align="center">
  <img height="500x" src="https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/images/infographics.png?raw=true" alt="summary">
</p>

### Target audience  ###
This specification is intended for:

* Data providers preparing DIGI4ECO datasets for publication in ERDDAP
* Software developers implementing metadata validation or ingestion pipelines
* End users who need to understand the structure and semantics of DIGI4ECO datasets
...
The attributes listed in the following table are expected to be included as global attributes in all DIGI4ECO-compliant
...
| keywords<sup>1</sup>            | List of keywords, for more info check [DIGI4ECO Keywords](https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/keywords.md)                                            | valid_keyword            | true     | true     |
| keywords_uri                    | List of keywords, for more info check [DIGI4ECO Keywords](https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/keywords.md)                                            | valid_keyword_uri        | true     | true     |
...
| project_code                    | List of the project identifiers                                                                                                                                                   | data_type#str            | false    | true     |
| project_uri                     | List of URIs pointing to autoritative records of projects, e.g. CORDIS                                                                                                            | data_type#str            | false    | true     |
...
| contributor_role                | role for each author following the [DataCite contributor type](https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/external-resources/datacite/DataCite_codes.md) | contributor_types        | true     | true     |
| doi                             | Digital Object Identifier (DOI) list of the dataset.                                                                                                                              | valid_doi                | true     | true     |
| data_processing_level           | Data processing level following [DIGI4ECO data levels](https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/Data_Processing_Levels.md).                                | valid_data_level         | false    | false    |
| data_processing_steps           | Data processing steps following [DIGI4ECO data levels](https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/Data_Processing_Levels.md).                                | valid_data_process       | false    | true     |
...
# Variables #
This section describes the different types of variables used in DIGI4ECO-compliant datasets and how they work together to 
...
<p align="center">
  <img height="350" src="https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/images/variables.jpg?raw=true" alt="variables">
</p>
...
In the DIGI4ECO metadata specification, coordinate variables are variables whose primary role is to describe the 
...
machine-readable geophysical datasets. In DIGI4ECO datasets, coordinate variables explicitly define the dimensional
...
<sup>5</sup> `standard_name` is not required for `sensor_id` and `time_end` since there is not an appropriate term in [CF Standard Name Table](https://cfconventions.org/Data/cf-standard-names/current/build/cf-standard-name-table).  
...
Since the `time_end` coordinate does not have a proper P01 term, the `sdn_parameter_name`, `sdn_parameter_urn` and `sdn_parameter_uri` are not required (blank string suggested).
...
The `time` variable represents the start of the sampling or integration period, and the `time_end` variable represents the end of that period. When both variables are present, 
each row describes data aggregated or measured over the interval [`time`, `time_end`]. This convention is used for observations derived over a finite duration, such as acoustic
recordings, video samples, or quantities computed over a time window (e.g. Sound Pressure Level).

The `time` and `time_end variables use the same units and reference time.

There is no corresponding term for time_end in the CF standard name table or in the BODC P01 vocabulary, so no controlled vocabulary entry is assigned to this variable.

## Data Variables ##
In the DIGI4ECO metadata specification, data variables are variables that contain the actual observed or derived values 
...
comparison, and integration of DIGI4ECO observations within operational oceanography and long-term climate applications.
...
responsible for acquiring the observations in an DIGI4ECO dataset. Rather than representing measured values, these variables
...
observations, such as fixed observatories, moorings, buoys, landers, or mobile platforms. In the DIGI4ECO metadata 
...
* **email**: valid email
* **oceansites_sensor_orientation**: A valid value from [OceanSITES sensor_orientation table](https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/external-resources/oceansites/OceanSites_codes.md)
* **oceansites_sensor_mount**: A valid value from [OceanSITES sensor_mount table](https://github.com/digi4eco/digi4eco-metadata-specifications/blob/main/external-resources/oceansites/OceanSites_codes.md)
